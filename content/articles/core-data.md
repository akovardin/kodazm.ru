...
Определимся с структурой данных которыми буду манипулировать в приложении. Для этого в файл _Memo.xcdatamodeld_ и добавлю новую сущность `Todo`. Для этого надо кликнуть на большой жирный плюс _Add Entity_

![](/img/core-data/generate5.png)

Теперь к новой сущности добавляем нужные аттрибуты. Для простого туду достаточно добавить `text`, `done` и поля `id` м типом UUID(это поле будет использоваться как уникальный идентификатор объекта).
